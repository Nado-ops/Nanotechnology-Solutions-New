...
export function LeadForm({ kind = 'contact' }: { kind?: 'contact' | 'support' }) {
  ...
  return <form className="lead-form" onSubmit={submit}>
    ...
    <div className="field-grid"><label>{kind === 'support' ? 'Support type' : 'Service required'} *<select name="service" required defaultValue=""><option value="" disabled>Select an option</option>{kind === 'support' ? <><option>Remote support</option><option>Onsite support</option><option>Existing client support</option><option>Urgent assistance</option></> : <><option>Point of Sale Systems</option>{solutions.map(s => <option key={s.slug}>{s.title}</option>)}</>}</select></label><label>Preferred contact method<select name="contactMethod"><option>Email</option><option>Phone</option></select></label></div>
